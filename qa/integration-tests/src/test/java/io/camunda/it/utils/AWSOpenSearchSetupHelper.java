...
public class AWSOpenSearchSetupHelper implements MultiDbSetupHelper {
  ...
  public AWSOpenSearchSetupHelper(
    ...
    expectedDescriptors.stream().map(d -> d.getFullQualifiedName()).forEach(dn ->
        LOGGER.debug("Expected descriptor: {}", dn)
    );
    LOGGER.debug("Amount of expected descriptors: {}", expectedDescriptors.size());
  ...
  protected int getCountOfIndicesWithPrefix(final String testPrefix)
    ...
    final var idxs =
        client
            ...
            .result();
    LOGGER.debug("Total indices found: {}", idxs.size());
