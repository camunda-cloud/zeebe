...
import ButtonBase from "@carbon/react/lib/components/Button/ButtonBase";
...
it('should render the table properly', () => {
  ...
  const task1ViewDetails = shallow(tableBody[0]?.[3] as JSX.Element);
  expect(task1ViewDetails.text()).toBe('View details');
  expect(task1ViewDetails.type()).toBe(ButtonBase);
  ...
  const task2ViewDetails = shallow(tableBody[0]?.[3] as JSX.Element);
  expect(task2ViewDetails.text()).toBe('View details');
  expect(task2ViewDetails.type()).toBe(ButtonBase);
});
